...
func getPresent() {
	...
	benchhelper.WriteMDFile("bench_get_present.md", results)
...
`
	...
	benchhelper.Fplot("bench_get_present.jpg", script)
}
...
func getAbsent() {
	...
	benchhelper.WriteMDFile("bench_get_absent.md", results)
	benchhelper.WriteDataFile("bench_get_absent.data",
		[]string{"key-count", "k=64", "k=128", "k=256"},
		results)

	script := `
fn = "bench_get_absent.data"
set yr [50:250]
set xlabel 'key-count (n)'
set ylabel 'Get() present key ns/op' offset 1,0
`
	...
	benchhelper.Fplot("bench_get_absent.jpg", script)
}
